...
class MyDataSet(data.Dataset):
	def __init__(self,floderPath,width,height,max_len,label_file):
		...
		file1 = open(self.label_file,"r")
		self.labels = file1.readlines()
	...
	def __getitem__(self,index):
		...
		txt_name = self.labels[int(os.path.basename(imgPath).replace('.jpg', '').split('_')[-1])-1][:-1]
